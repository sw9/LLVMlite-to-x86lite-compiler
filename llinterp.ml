...
let interp_prog {tdecls; gdecls; fdecls} (args:string list) : sval =
  ...
  let rec interp_cfg (k, blocks:cfg) (locs:locals) (c:config) : config * sval =
    match k.insns, k.terminator with
